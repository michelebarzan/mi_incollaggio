<?php
	...
	disegnaAngolo($conn,$codPan);
	disegnaLana($conn,$codPan);
	
	function disegnaLana($conn,$codPan)
	{
		$LUNG='';
		$Pos=array();
			
		$queryLana="SELECT * FROM Pc_xlana_nofiltro WHERE LUNG=(SELECT MAX(LUNG) FROM Pc_xlana_nofiltro WHERE codpan='$codPan') AND codpan='$codPan'";
		...
		$queryLanaFresature="SELECT * FROM xlana_fresature_nofiltro_ruotato  WHERE codpan='$codPan'";
		...
		echo '<svg height="450px" width="100%">';
			...
			echo '<text x="'.(120+$HALT).'" y="'.(420-($LUNG/2)).'" transform="rotate(270 '.(120+$HALT).','.(420-($LUNG/2)).')" fill="black" style="font-family:Arial;font-weight:bold;font-size:130%" >'.(round(($LUNG*2),1)).'</text>';
			
		echo 'Sorry, your browser does not support inline SVG.</svg>';
	
	}
	function disegnaAngolo($conn,$codPan)
	{
		...
		if($nRighe>=2)
		{
			...
			echo '<svg height="250px" width="100%">';
				//PANNELLO
				echo '<rect x="50" y="'.$Y_rettangolo1.'" width="'.$HALT.'" height="'.$LUNG.'" style="fill:white;stroke:black;stroke-width:2" />';
				echo '<text x="30" y="'.($Y_rettangolo1-10).'" fill="black" style="font-family:Arial;font-weight:bold;font-size:130%" >'.$Ang1.'°</text>';
				echo '<text x="30" y="'.($Y_rettangolo1+$LUNG+25).'" fill="black" style="font-family:Arial;font-weight:bold;font-size:130%" >'.$Ang2.'°</text>';
