<?php
	...
	$queryRighe="SELECT COUNT(*) AS nRighe FROM Pc_xlana";
	...
	$queryLana="SELECT * FROM Pc_xlana WHERE LUNG=(SELECT MIN(LUNG) FROM Pc_xlana)";
	...
	else
	{
		while($rowLana=sqlsrv_fetch_array($resultLana))
		{
			...
			$Ang1=$rowLana['Ang1'];
			$Ang2=$rowLana['Ang2'];
			$latoStretto=$rowLana['latoStretto'];
			if($latoStretto>0)
				$latoStretto='+'.$latoStretto;
			if($latoStretto==0)
				$latoStretto='';
		}
	}
	...
	echo '<svg height="250px" width="100%">';
		...
		echo '<text x="'.(120+$HALT).'" y="'.(220-($LUNG/2)).'" transform="rotate(270 '.(120+$HALT).','.(220-($LUNG/2)).')" fill="black" style="font-family:Arial;font-weight:bold;font-size:130%" >'.(round(($LUNG*2),1)).$latoStretto.'</text>';
